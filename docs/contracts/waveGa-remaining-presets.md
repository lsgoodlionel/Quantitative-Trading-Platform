...
3. **非因果指标改不了。** `indicators.py` 的 `ichimoku` 有
   ...
   碰到这类只能保留原写法。
4. **`supertrend` 一类的递归指标要特别小心**：它依赖前一根的状态。
   预算一次算完与逐 bar 递推在**热身段**可能不同 —— 如果对拍不过，
   那正是这条要防的，保留原写法。
...
⚠️ **`pairs_trading` 是组合策略**（跨标的），它的上下文是 `PortfolioContext`，
指标要走 `ctx.ind(symbol)` 而不是 `ctx.ind`。别按单标的范式套。

⚠️ **`multi_factor` 可能引用因子库而非 `indicators.py`**，先读清楚它的依赖再动。
