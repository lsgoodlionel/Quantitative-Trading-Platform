...
> **2026-08-11 开工前复核**：本表若干项已被前序 Wave 做掉或部分做掉，照原表实施会做重复功。
>
> | 特性 | 原记 | 复核后 |
> |---|---|---|
> | **N5 主动式偏差检测** | 待做 | ✅ **已完整实现** —— `bias_detection.py` 的 `detect_lookahead` + `detect_recursive`，文件头即注明参考 freqtrade `recursive.py`。原表把它记成缺口是过时的 |
> | **N3 回撤恢复期** | 待做 | ✅ 已有 `drawdown_periods.py`（peak→valley→recovery）；**仅缺危机区间分段对照** |
