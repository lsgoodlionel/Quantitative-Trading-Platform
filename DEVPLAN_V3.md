...
- **Wave D**：J5 生产加固（配置校验/健康检查/安全响应头）· Playwright E2E · Nginx 反代/备份/负载测试
- **Wave E**：J2（重定向为指标预算，单标的 6.6×、组合 16.4×）
- **J4**：已出评估结论 —— 不启用 torch
...
| J4 torch 评估 | **已交付（结论：不启用）** | 见 [docs/torch-evaluation-j4.md](docs/torch-evaluation-j4.md)。torch linux wheel 527MB（ARM64 427MB），而当前全部依赖合计才 505MB —— 加它等于把现有依赖再装一遍还多，换 ARM64 也解决不了。收益未经证实：平台已有 GBDT 路径，且 sequence.py 的训练/预测集耦合缺陷未修，装上也产不出最新信号。现状「装了能用、不装不碍事」已是最优形态 |
